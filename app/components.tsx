...
export function Shell({ active, children }: { active: string; children: React.ReactNode }) {
  ...
  return <>
    ...
    <main>{children}</main>
    <footer><div><strong>Intelligence News Portal</strong><span>AI · UAV · ROBOT WEEKLY</span></div><p>為產業決策者設計的每週情報入口 · 更新日期 2026.09.12</p></footer>
  </>;
}
...
export function NewsPage({ active, title, label, intro, items }: { active: string; title: string; label: string; intro: string; items: NewsItem[] }) {
  return <Shell active={active}>
    <section className="page-hero"><p className="eyebrow">WEEKLY BRIEFING · {label}</p><h1>{title}</h1><p>{intro}</p><div className="page-meta"><span>本週 {items.length} 則精選</span><span>更新 2026.09.12</span><span>約 8 分鐘閱讀</span></div></section>
